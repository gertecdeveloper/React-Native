...
export default class ConfigSat extends Component{
    ...
    async componentDidMount(){
        DeviceEventEmitter.addListener("eventRede",event=>{ this.formatarInfo(event.rede)});
    ...
    formatarInfo(retorno){
        ...
        // Instância a classe Dinamica, responsavel por formata o retorno do Sat ao invocar "Configuração Rede" Sat.
        // Com este objeto é possivel acessar as informações recebidas do Sat, de uma forma simplificada.
        var retornoConfigSat = new RetornoDinamicoSat(retorno);
        this.dialogoSat(retornoConfigSat.getRespostaRecebida());
    ...
    configRede(){
        if(this.state.codigoAtivacao.length>=8 && this.state.codigoAtivacao.length<=32){
            ToastExample.configRede(Math.floor(Math.random() * 9999999), this.state.codigoAtivacao,this.formatarEnvioConfigRede())
        }else{
